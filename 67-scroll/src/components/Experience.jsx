...
export const Experience = () => {
  const scrollData = useScroll(); // Get the scroll data
  useFrame((state) => {
...
const FoodItem = ({ model, page }) => {
  ...
  const viewport = useThree((state) => state.viewport); // Get the viewport
  ...
  useFrame(() => {
    ...
    foodItemRef.current.rotation.y = pageScroll * Math.PI * 2;
    ...
    foodItemRef.current.position.x =
      scrollData.curve((page - 1) / pages, 2 / pages) * offsetX;
  });

  return (
    <group ref={foodItemRef}>
      <primitive
        object={gltf.scene}
        position={[0, -viewport.height * page, 0]} // Position the model based on the page
      />
    </group>
  );
};
